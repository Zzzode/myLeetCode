 ...
 * Testcase Example:  '1\n2\n0\n0'
 *
 * 给出 R 行 C 列的矩阵，其中的单元格的整数坐标为 (r, c)，满足 0 <= r < R 且 0
 * <= c < C。
 *
...
class Solution {
 public:
  vector<vector<int>> allCellsDistOrder(int R, int C, int r0, int c0) {
    vector<vector<vector<int>>> vvp;
    int n = max(r0 - 0, R - r0) + max(c0 - 0, C - c0);
    vvp.resize(n + 1);
    for (int i = 0; i < R; i++) {
      for (int j = 0; j < C; j++) {
        vvp[abs(i - r0) + abs(j - c0)].push_back({i, j});
      }
    }
    vector<vector<int>> ans;
    for (int i = 0; i <= n; i++) {
      if (!vvp[i].empty()) {
        for (auto& v : vvp[i]) ans.push_back(v);
      }
    }
    return ans;
  }
