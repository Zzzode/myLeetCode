 ...
 * 输入: [1,4], 2
 * 输出: 4
 * 原因: 第 1 秒初，提莫开始对艾希进行攻击并使其立即中毒。中毒状态会维持 2
 * 秒钟，直到第 2 秒末结束。 第 4 秒初，提莫再次攻击艾希，使得艾希获得另外 2
 * 秒中毒时间。 所以最终输出 4 秒。
 *
 *
 * 示例2:
 ...
 * 原因: 第 1 秒初，提莫开始对艾希进行攻击并使其立即中毒。中毒状态会维持 2
 * 秒钟，直到第 2 秒末结束。 但是第 2
 * 秒初，提莫再次攻击了已经处于中毒状态的艾希。 由于中毒状态不可叠加，提莫在第 2
 * 秒初的这次攻击会在第 3 秒末结束。 所以最终输出 3 。
 *
 *
 *
 *
...
class Solution {
 public:
  int findPoisonedDuration(vector<int>& timeSeries, int duration) {
    ...
    if (!n) return 0;
    for (int i = 0; i < n - 1; i++) {
      int t0 = timeSeries[i], t1 = timeSeries[i + 1];
      ans += t0 + duration - 1 >= t1 ? t1 - t0 : duration;
    }
    ans += duration;
